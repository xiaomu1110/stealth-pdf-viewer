...
[![Version](https://img.shields.io/badge/Version-v1.5.3-success.svg)](https://github.com/xiaomu1110/stealth-pdf-viewer/releases)
...
- **上传**：打开题册后，状态栏点击「同步」→ 选择上传，将当前 PDF 与做题笔记推送到 Gitee 仓库 `StealthPDFSync/` 目录。
- **新电脑拉取（云端书库）**：换新电脑无需预先放置本地文件！按 `Ctrl+Shift+P` 执行 `Stealth PDF: 从云端拉取题册 (云端书库)`（或未打开 PDF 时直接点击状态栏「同步」），浏览云端题册列表，选择保存位置即可一键下载、自动恢复笔记并即刻打开。
- **覆盖拉取**：打开本地题册时点击状态栏「同步」→ 选择拉取，云端版本覆盖本地并自动恢复笔记（跨电脑路径不一致时支持按文件名智能匹配）。
...
### 方式 B：终端一键安装
```bash
code --install-extension stealth-pdf-viewer-1.5.3.vsix
```
